...
def compute_robot_logic(walls, goal, bot):
    ...
    selected_move = moves[rand.randint(0, 3)] # random a way (maybe slowest, maybe fastest)
    move_dist = []
    for m, move in enumerate(moves):

        # Calculate the distance between the bot and the goal
        # if move left, rightt, down, up
    ...
    if rand.random() < 0.45: # if rand.random() < 45: then choose the fatest way
        selected_move = moves[move_dist[0][0]]

    
    hit_wall = False
    for wall in walls: # Loop through every wall to see if it's hit or not
        if bot.calc_x + selected_move[0] == wall.x and bot.calc_y + selected_move[1] == wall.y:
            hit_wall = True
            
    found_alternate_move = False
    if hit_wall: # If hit_wall then choose the fastest way
        for next_move in move_dist:
            ...
            if not hit_wall_move: # If not hit then choose that move
                selected_move = move
                found_alternate_move = True
                break

        if not found_alternate_move: # If hit stay put
            selected_move = (0, 0)
